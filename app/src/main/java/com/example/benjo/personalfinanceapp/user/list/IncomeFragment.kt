...
class IncomeFragment : BaseFragment() {
    ...
    override fun attachTouchHelper(recyclerView: RecyclerView?) {
        val touchCallback = object : ItemTouchHelper.SimpleCallback(0, ItemTouchHelper.LEFT or ItemTouchHelper.RIGHT) {
            ...
            override fun onSwiped(viewHolder: RecyclerView.ViewHolder?, direction: Int) {
                presenter!!.deleteTransaction(AnkoDB.TABLE_INCOME, viewHolder!!.itemView.tag as Int)
            }
        }
    ...
    override fun onResume() {
        super.onResume()
        transactions_fab.setOnClickListener { presenter!!.show(Constants.INCOME_TRANSACTION) }
        presenter!!.loadAllTransactions(AnkoDB.TABLE_INCOME)
    }
